...
func (c *EC2SDConfig) NewDiscoverer(opts discovery.DiscovererOptions) (discovery.Discoverer, error) {
	return NewEC2Discovery(c, opts.Logger, opts.DebugMetrics), nil
}
...
func NewEC2Discovery(conf *EC2SDConfig, logger log.Logger, metrics discovery.DiscovererDebugMetrics) *EC2Discovery {
	m, err := convertToEc2Metrics(metrics)
	if err != nil {
		return nil
	}
